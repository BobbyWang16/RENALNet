...
def train_model(model, train_loader, test_loader, sets):
    # 分出训练集和验证集
    # train_loader, val_loader = train_test_split(train_loader, test_size=0.2, random_state=42)
    ...
    for epoch in range(num_epochs):
        ...
        # 打印训练信息
        print(f'Train Epoch: [{epoch+1} / {num_epochs}], Step: [{batch_idx+1} / {len(train_loader)}], Total_Loss: {total_loss}, LR: {scheduler.get_last_lr()}, Test_AUC: {test_auc}, Train_auc: {train_auc}')
        # 每隔1轮保存一次模型
...
def set_seed(seed=12):
    ...
    torch.backends.cudnn.benchmark = False  # True的话会自动寻找最适合当前配置的高效算法，设置为False可以保证实验结果的可重复性
...
def main():
    ...
    # 训练
    sets.stage = 'train'
    # sets.gt_path = "./data/gt/{}.csv".format(sets.phase)
    sets.batch_size = 16
    sets.n_epochs = 100
    sets.learning_rate = 0.0001
    sets.save_intervals = 1
    sets.save_folder = "./checkpoint/chk_5"
    print(sets.save_folder)
    # sets.save_folder = "./checkpoint/{}_{}/chk{}".format(sets.model, sets.model_depth, "_2")
